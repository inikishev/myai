...
class GSolver(Optimizer):
    ...
    But it is prone to getting stuck, in this version. Maybe hyperparameters could be tuned."""
    ...
    def step(self, closure):
        ...
        with torch.no_grad():
            ...
            if state['initial_phase']:
                # Generate m_initial random perturbations
...
class GSolverV2(torch.optim.Optimizer):
    def __init__(self, params, lr=1e-3, num_init=10, memory_size=10,
        ...
        self._state = {}  # State management using group IDs
    ...
    def step(self, closure):
        ...
        for group in self.param_groups:
            ...
            if step < num_init:
                # Initial phase with gradient clipping
                ...
                # Gradient clipping during initialization
                ...
                # Main optimization phase with step rejection
                with torch.no_grad():
                    # Clamp gradient estimates
                    grad_norm = torch.norm(grad_est)
                    if grad_norm > max_grad_norm:
                        grad_est.mul_(max_grad_norm / (grad_norm + eps))

                    # Apply stabilized step
                    ...
                    # Improved two-loop recursion with validity checks
                    ...
                    # First loop with validity checks
                    ...
                    # Second loop with gradient clipping
                    z = q.clone()
                    ...
                    # Update gradient estimate with clamped momentum
                    ...
                    # Controlled exploration with adaptive scaling
                    if step % exploration_interval == 0:
                        ...
                        # Adaptive exploration scale based on recent progress
